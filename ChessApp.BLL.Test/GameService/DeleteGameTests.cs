...
namespace ChessApp.BLL.Tests
{
    ...
    public class DeleteGameTests
    {
        private static (Mock<IUOW> uow, Mock<IGameRepository> gameRepo,
            Dictionary<int, Game> dbCollectionGame) GetMocks()
        {
            ...
            gameRepo.Setup(e => e.Remove(It.IsAny<Game>()))
                     .Callback((Game newGame) => { dbCollectionGame.Remove(newGame.Id); });
            ...
            playerRepo.Setup(e => e.Remove(It.IsAny<Player>()))
                      .Callback((Player newPlayer) => { dbCollectionPlayers.Remove(newPlayer.Id); });

            return (uow, gameRepo, dbCollectionGame);
        }

        [Test]
        public async Task DeleteGame_TargetItem_Success()
        {
            // Arrange
            ...
            // Act
            await service.DeleteGame(game);

            // Assert
            Assert.IsFalse(dbCollectionGame.ContainsKey(26));
        }

        [Test]
        public void DeleteGame_ItemDoesNotExists_NullReferenceException()
        {
            // Arrange
            var (uow, gameRepo, dbCollectionGame) = GetMocks();
            var service = new GameService(uow.Object);
            ...
            // Act + Assert
            Assert.ThrowsAsync<NullReferenceException>(async () => await service.DeleteGame(game));
        }
    }
}